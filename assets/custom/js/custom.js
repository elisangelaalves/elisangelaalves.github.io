            ...
            if ($('[data-section="feedback"]').length) {
                ...
                $(form).submit(function (e) {
                    ...
                    .done(function (response) {
                        var lang = ((window.location.pathname.search("/en")) == -1 ? 'br' : 'en');
                        if (response == 'success') {
                          // Make sure that the formMessages div has the 'success' class.
                          $(formMessages).removeClass('alert alert-danger');
                          $(formMessages).addClass('alert alert-success');
                          ...
                          // Set the message text.
                          if (lang == 'en') {
                            $(formMessages).text("Your message was sent.");
                          } else {
                            $(formMessages).text("Sua mensagem foi enviada.");
                          }
                          ...
                          if (lang == 'en') {
                            if (response.errorMessage == 'invalid_name') {
                                $(formMessages).text("Your message has not been sent. Please, check your name.");
                            } else if (response.errorMessage == 'invalid_email') {
                                $(formMessages).text("Your message has not been sent. Please, check your email.");
                            } else if (response.errorMessage == 'invalid_phone') {
                                $(formMessages).text("Your message has not been sent. Please, check your phone.");
                            } else if (response.errorMessage == 'invalid_message') {
                                $(formMessages).text("Your message has not been sent. Please, check your message.");
                            } else {
                                $(formMessages).text("Error: Your message has not been sent. Try again later.");
                            }
                          } else {
                            if (response.errorMessage == 'invalid_name') {
                                $(formMessages).text("Mensagem n\xE3o enviada. Verifique seu nome.");
                            } else if (response.errorMessage == 'invalid_email') {
                                $(formMessages).text("Mensagem n\xE3o enviada. Verifique seu email.");
                            } else if (response.errorMessage == 'invalid_phone') {
                                $(formMessages).text("Mensagem n\xE3o enviada. Verifique seu telefone.");
                            } else if (response.errorMessage == 'invalid_message') {
                                $(formMessages).text("Mensagem n\xE3o enviada. Verifique sua mensagem.");
                            } else {
                                $(formMessages).text("Erro: Mensagem n\xE3o enviada. Tente novamente mais tarde.");
                            }
                          }
                        }
                    })
